"""
Matplotlib-based live visualisation for Stage 2 grid world.
Adds trap and traffic rendering on top of the base city grid.
...
import numpy as np
import matplotlib
matplotlib.use('TkAgg')         # interactive backend for GUI display
...
def _draw_background(ax, grid, goal):
    ...
    for r in range(size):
        for c in range(size):
            if grid[r, c] == 1:
                display[r, c] = [0.22, 0.22, 0.26]     # wall
            else:
                display[r, c] = [0.93, 0.93, 0.90]     # road
...
def show_grid_s2(grid, goal, agent_pos=None, path=None,
                 traps=None, traffic_positions=None,
                 title='Stage 2', pause=0.02):
    """
    Render one frame.
    traps              : set of (r, c)
    traffic_positions  : list of (r, c)
    """
    ...
    time.sleep(pause)  # Allow frame to display and process events


def save_frame(path="logs/stage2_frame.png", dpi=120):
    """Save current figure to disk (useful for headless snapshots)."""
    global _fig
    if _fig is not None:
...
def reset_view_s2():
    global _agent_dot, _path_line, _trap_patches, _traffic_patches
    if _agent_dot is not None:
        _agent_dot.remove()
        _agent_dot = None
    if _path_line is not None:
